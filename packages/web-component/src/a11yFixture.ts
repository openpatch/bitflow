...
export const steps = [
  ...
  node("q-free-text", "task-free-text", {
    ...
    criteria: [
      {
        id: "sorted",
        label: "Mentions sorted order",
        keywords: ["sorted", "order"],
        points: 1,
      },
    ],
    modelAnswer: "",
  ...
  node("q-boolean", "task-boolean-logic", {
  ...
  node("q-annotate", "task-image-annotation", {
    instruction: "Mark the middle of the square.",
    background: { src: "data:image/png;base64,AAAA", alt: "A square on a plain background" },
    size: { width: 620, height: 310 },
  ...
  node("q-table", "task-table", {
    instruction: "What does the query return?",
    caption: "Result",
    columns: [
      { id: "name", header: "name", kind: "text" },
      { id: "age", header: "age", kind: "number" },
    ],
    rows: [
      { id: "r1", cells: { name: { given: "Ada" }, age: { accepted: ["16"] } } },
    ],
    rowHeaders: false,
    rowOrder: "any",
    caseSensitive: false,
  ...
  node("q-plot", "task-point-plot", {
    instruction: "Assign the point to its nearest centre.",
  ...
  node("q-cardinality", "task-cardinality", {
    instruction: "A pupil borrows many books; a book is borrowed by one pupil.",
    notation: "chen",
    entities: [
  ...
  node("q-numberline", "task-number-line", {
    instruction: "Place the numbers on the line.",
    min: -2,
    max: 2,
    tickStep: 1,
    minorTicks: 3,
  ...
  node("q-functionplot", "task-function-plot", {
    instruction: "Sketch f(x) = 0.5x^2 - 2.",
    axes: {
