 ...
 *
 * There is no assignment, no property access and no way to name anything the
 * allow-lists below do not already name. A caller may name variables of its
 * own — a function plot evaluates `x^2 - 1` at a given `x` — and only when it
 * does may a number stand directly before a name or a bracket to multiply it
 * (`3x`, `2(x + 1)`), since that is how a function is written; an answer box
 * without variables keeps refusing `2pi` as the syntax error it is there.
 *
 * Lives in core, not in a bit, because more than one bit reads arithmetic:
 * task-numeric for answers, task-function-plot for the function to draw.
 */
...
export type ParseOptions = {
  ...
  /**
   * Names the caller gives values to, like `x`. A name here is read before a
   * constant of the same spelling. Giving any also turns on multiplication by
   * juxtaposition: `3x` is `3*x`.
   */
...
const tokenize = (
  ...
  while (index < source.length) {
    ...
    if (isNameStart(character)) {
      ...
      const isVariable = Object.hasOwn(options.variables, name);
      if (!opensCall && !isConstant && !isVariable) {
...
class Parser {
  ...
  /** Whether a number may multiply what follows it without a `*`. */
  private get juxtaposes(): boolean {
    return Object.keys(this.variables).length > 0;
  }

  /** Whether the next token begins a factor on its own: `x`, `2`, `(`. */
  ...
  private term(): number {
    ...
    for (;;) {
      if (this.juxtaposes && this.startsFactor()) {
        value *= this.power();
        continue;
      }
      const operator = this.eatOperator("*", "/");
  ...
  private primary(): number {
    ...
    if (token.kind === "name") {
      this.position++;
      if (Object.hasOwn(this.variables, token.text)) return this.variables[token.text];
...
export const parseExpression = (
  ...
  const parser = new Parser(tokens, settings.variables);
  let value: number;
  try {
